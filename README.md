...
安装程序会自动尝试以下来源，不需要手动配置镜像：

| 环节 | 优先来源 | 后续兜底 |
|---|---|---|
| Python 3.12 | 中国科学技术大学镜像 | GitHub 官方源 |
| GPU 推理依赖 | 中国科学技术大学 PyPI | 清华 PyPI、官方 PyPI |
| WD14 模型 | Hugging Face | 备用镜像与 Windows 下载通道 |

下载失败时会保留可复用的缓存并自动切换来源。
...
进入插件“设置 → 导入本地模型”，同时选择这两个文件。插件会检查固定文件大小和 SHA-256，完全匹配后才写入模型缓存；失败不会破坏已有模型。
...
公开测试包的使用与再分发边界见 [LICENSE](LICENSE)。此前已经随 MIT License 发布的 0.7.0 及更早版本，继续适用其发布时的许可。
